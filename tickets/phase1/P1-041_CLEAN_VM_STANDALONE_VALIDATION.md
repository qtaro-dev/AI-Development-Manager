...
## P1-041初回試験使用成果物（P1-040 Release生成物）
...
## P1-043修正版クリーンVM再試験結果

P1-043でWebAssets収録・探索パス・MSI標準UIを是正した版を使用して再試験を実施した。P1-041の初回試験結果は履歴として保持し、今回の結果を追記する。

| 項目 | 値 |
|---|---|
| MSI | `D:\Dev\AI Development Manager\artifacts\packages\client\AI-Development-Manager-Client-0.1.0-1-x64.msi` |
| SHA-256 | `F819AC2EA98D8CD6F868A28B5502256AD740556AA4307B98FA3F2B3B23AEC48A` |
| VMへコピーする正式試験成果物 | MSIのみ（`manifest.json`はインストールに不要な記録用ファイル） |

今回の再試験で確認された未解決事項は次のとおり。

| 区分 | 結果 | 判定・対応 |
|---|---|---|
| 初回設定 | 初回設定の保存、再読込、保存後の画面遷移に是正事項あり | P1-044へ分離。P1-041では修正しない |
| Client MSI | 日本語化および正式なLicense内容に是正事項あり | P1-045へ分離。P1-041では修正しない |

P1-044／P1-045の実施時期と優先順位はPhase 2開始時に判断する。今回、Server、Windows Service、LAN接続の追加試験・修正は行っていない。

## 状態

再試験実施。P1-044／P1-045是正待ち。
