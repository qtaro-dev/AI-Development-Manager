...
初回全走査と定期・手動再走査を正とし、FileSystemWatcherはデバウンスした作業キューとして扱う。走査スナップショットの差分で追加、変更、改名、削除を補正し、読込中ファイルは上限付き再試行後に文書単位エラーとする。1文書の失敗で全走査を停止せず、キャンセル・再開・進捗・ヘルス状態を記録する。
...
P0-003の負荷モデルに基づき、1万文書・人5・AIクライアント2を初期性能基準とする。P0-015では候補一覧は暫定3秒を満たした一方、初回走査p95 5,141.1ms、索引済み検索p95 945.2msとなり、暫定目標を未達とした。走査の増分化、索引投入方式、Tokenizer・容量の見直しをPhase 1改善候補として扱い、正確性を優先する。正式性能値はP0-023で確定する。
