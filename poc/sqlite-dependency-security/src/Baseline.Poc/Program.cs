...
var result = Probe("baseline", "Microsoft.Data.Sqlite 10.0.10");
var output = Path.Combine(Path.GetTempPath(), "AI-Development-Manager", "poc", "P0-026", result.RunId);
Directory.CreateDirectory(output);
File.WriteAllText(Path.Combine(output, "baseline-result.json"), JsonSerializer.Serialize(result, new JsonSerializerOptions { WriteIndented = true }));
Console.WriteLine(JsonSerializer.Serialize(result, new JsonSerializerOptions { WriteIndented = true }));
Console.WriteLine($"RESULT_JSON={Path.Combine(output, "baseline-result.json")}");
